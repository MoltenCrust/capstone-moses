...
<template>
  <div class="p-4">
    <div class="flex align-items-center justify-content-between mb-4">
      <h2 class="text-xl font-semibold m-0">Items</h2>
    </div>
    ...
    >
      ...
      <Column field="item_code" header="Item Code" sortable style="width: 160px">
      ...
      <Column field="description" header="Description" sortable>
        ...
        <template #editor="{ data, field }">
          <InputText v-model="data[field]" fluid size="small" placeholder="optional" />
        </template>
      </Column>
...
// ── Row edit save → PUT /items/:id ────────────────────────────
async function onRowEditSave(event: DataTableRowEditSaveEvent) {
  const { newData, index } = event;
  try {
    const res  = await fetch(`${API_BASE}/items/${newData.id}`, {
      method:  'PUT',
    ...
    toast.add({ severity: 'secondary', summary: 'Saved', detail: 'Item updated', life: 3000 });
  } catch (err: any) {
